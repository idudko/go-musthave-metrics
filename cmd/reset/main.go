...
var resetTemplate = template.Must(template.New("reset").Parse(`// Reset resets the {{.Name}} fields to their zero values
func (x *{{.Name}}) Reset() {
	if x == nil {
		return
	}
...
func generateResetMethod(buf *bytes.Buffer, st *StructInfo) {
	// Регистрируем функции в шаблоне
	resetTemplate.Funcs(TemplateFuncs)
